...
from model_utils import frontend_3D, backend_resnet, stacked_lstm  # blstm_encoder, lstm_encoder
...
class Model3:
    ...
    def build_train_graph(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            # out_cell = tf.contrib.rnn.OutputProjectionWrapper(
            #     decoder_cell, self.options['num_classes'])
            #
            # out_cell = tf.contrib.seq2seq.AttentionWrapper(
            #     cell=out_cell,
            #     attention_mechanism=attention_mechanism,
            #     attention_layer_size=self.options['attention_layer_size'],
            #     alignment_history=False,
    ...
    def save_summaries(self, sess):
        s, gs = sess.run([self.merged_summaries, self.global_step])
